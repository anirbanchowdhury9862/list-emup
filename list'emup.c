...
if(argc!=2)
       {strcpy(argv[0],getcwd(ch,128));
        ...
        file_system = fts_open(argv+0,FTS_PHYSICAL|FTS_XDEV,&compare);
        }

else
    {file_system = fts_open(argv + 1,FTS_PHYSICAL|FTS_XDEV,&compare);
    }
            ...
            {strcpy(ch,parent->fts_name);
                ...
                if(strcmp(ch,parent->fts_name)!=0)
                 { fts_set(file_system,parent,FTS_SKIP);
                    if(f_namesort)
                    printf("\n%llu\t\t%s",parent->fts_statp->st_size,parent->fts_name);

                    if(f_statuschangesort)
                    printf("\n%s\t\t%s",ctime(&parent->fts_statp->st_ctime),parent->fts_name);

                    if(f_modificationsort)
                    printf("\n%s\t\t%s",ctime(&parent->fts_statp->st_mtime),parent->fts_name);

                    if(f_accesstimesort)
                    printf("\n%s\t\t%s",ctime(&parent->fts_statp->st_atime),parent->fts_name);
